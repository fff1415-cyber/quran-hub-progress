...
export function DailyOperations() {
  ...
  const scheduled = useMemo(() => queue.filter((q) => isSupervisorForceRetryCandidate(q)), [queue]);
  ...
  const forceImmediate = (item: SardQueueItem, name: string) => {
    ...
    pushNotification({
      message: kind === "review"
        ? `سمح ${me} بإعادة مراجعة فورية للطالب ${name}`
        : `سمح ${me} بإعادة سرد فوري للطالب ${name}`,
      type: "sard",
    });
  ...
  const renderSardCard = (item: SardQueueItem) => {
    ...
    if (item.status === "scheduled" || item.status === "awaiting_review") {
      const kind = forceRetryKind(item);
      return (
        <div key={item.id} className="flex items-center justify-between p-3 rounded-lg bg-warning/5 border border-warning/20">
          <div>
            ...
            <div className="text-xs text-muted-foreground mt-1">
              {h.name} · {weekLabel(item.week)} · {kind === "review" ? "انتظار إعادة المراجعة" : "انتظار إعادة السرد"}
            </div>
          </div>
          <button onClick={() => forceImmediate(item, s.name)}
            ...
            <Zap className="w-4 h-4" /> {kind === "review" ? "المراجعة الآن" : "الإعادة الآن"}
          </button>
        </div>
      );
    }
